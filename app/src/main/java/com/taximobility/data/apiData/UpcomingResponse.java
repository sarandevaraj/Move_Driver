package com.taximobility.data.apiData;

/**
 * Created by developer on 1/11/16.
 * Contains data for upcoming response
 */

import java.util.ArrayList;
import java.util.List;


public class UpcomingResponse {

    public String message;
    public Detail detail;
    public Integer status;

    public class Detail {
        public String cancelfare_applicable;
        public List<PastBooking> pending_bookings = new ArrayList<PastBooking>();
        public List<PastBooking> past_bookings = new ArrayList<PastBooking>();

    }

    public class PastBooking {

        public String passengers_log_id;
        public String pickup_location;
        public String drop_location;
        public String pickup_longitude;
        public String pickup_latitude;
        public String drop_longitude;
        public String drop_latitude;
        public String pickup_time;
        public String travel_status;
        public String trip_id;
        public String passenger_name;
        public String driver_id;
        public String notes_driver;
        public String drivername;
        public String drop_time;
        public String trip_duration;
        public String taxi_no;
        public String model_name;
        public String profile_image;
        public String waiting_fare;
        public String fare, map_image, payment_type;
        public String cancellation_fee;
        public String approx_fare;
        public String approx_distance;
        public String now_after;

        public String corporate_booking;
        public String place;

    }

}