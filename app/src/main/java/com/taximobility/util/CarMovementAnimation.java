...
public class CarMovementAnimation {
    ...
    public void addMarkerAnimate(final Marker marker) {
        ValueAnimator anis = ValueAnimator.ofFloat(0, 1);
        MyBounceInterpolator interpolator = new MyBounceInterpolator(0.1, 5);
        anis.setInterpolator(interpolator);
        anis.setDuration(2000);
        anis.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                marker.setAnchor(1f, (Float) valueAnimator.getAnimatedValue());
            }
        });
        anis.start();
    }
