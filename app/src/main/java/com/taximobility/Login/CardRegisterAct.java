...
public class CardRegisterAct extends MainActivity implements OnClickListener {
    ...
    ImageView back_img;
    private TextView promocodeTxt, cashTxt, creditcardTxt, leftIcon, HeadTitle, BackBtn;
    private TextView termsTxtCash, termscard, cancel_b;
    private CheckBox termscheck, isDefaultChk, termsTxt;
    private EditText cvvEdt, cardnameEdt, cardnumEdt;
    private LinearLayout carddetail_lay, cashLayout, Donelay;
    private Spinner monthspn, yearspn;
    private Button submitBtn;

    private Dialog mDialog, dialog;
    private String[] monthLst, yearLst;
    private String expmonth = "", expyear = "";
    private boolean monthtouched, yeartouched;
    private int curmonth, curyear, isDefault = 0;
    ...
    public static String buildCorrecntString(char[] digits, int dividerPosition, char divider) {
    ...
    public void onBackPressed() {
        ...
        final Intent i = new Intent(CardRegisterAct.this, DriverUserLoginAct.class);
        startActivity(i);
        finish();
    }
    ...
    public void Initialize() {
        ...
        SessionSave.saveSession(TaxiUtil.NEED_TO_COMPLETE_CARD_REG, true, CardRegisterAct.this);
        ...
        creditcardTxt = findViewById(R.id.creditcardTxt);
        cashTxt = findViewById(R.id.cashTxt);
        promocodeTxt = findViewById(R.id.promocodeTxt);
        carddetail_lay = findViewById(R.id.carddetail_lay);
        cashLayout = findViewById(R.id.cashLayout);
        monthspn = findViewById(R.id.monthspn);
        yearspn = findViewById(R.id.yearspn);
        cardnumEdt = findViewById(R.id.cardnumEdt);
        cvvEdt = findViewById(R.id.cvvEdt);
        cardnameEdt = findViewById(R.id.cardnameEdt);
        termsTxt = findViewById(R.id.termsTxt);
        termsTxtCash = findViewById(R.id.termsTxtCash);
        termscard = findViewById(R.id.termscard);
        termscheck = findViewById(R.id.termscheck);
        isDefaultChk = findViewById(R.id.isDefaultChk);
        submitBtn = findViewById(R.id.submitBtn);
        monthLst = getResources().getStringArray(R.array.monthlistary);
        yearLst = getResources().getStringArray(R.array.yearary);
        findViewById(R.id.emergency_lay).setVisibility(View.GONE);
        LinearLayout layoutSkip = findViewById(R.id.layout_skip);
        layoutSkip.setVisibility(View.VISIBLE);
        cancel_b = findViewById(R.id.txt_skip);

        cancel_b.setVisibility(View.VISIBLE);
        cancel_b.setText(NC.getString(R.string.skip_));

        cancel_b.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                callSkipApi();
                SessionSave.saveSession(TaxiUtil.NEED_TO_COMPLETE_CARD_REG, false, CardRegisterAct.this);
        ...
        if (SessionSave.getSession("SKIP_CREDIT_CARD", CardRegisterAct.this, false)) {
            cancel_b.setVisibility(View.VISIBLE);
        } else
            cancel_b.setVisibility(View.GONE);
        int selection_yearpos = 0;
        for (int i = 0; i < yearLst.length; i++) {
            if (yearLst[i].equalsIgnoreCase(String.valueOf(curyear))) {
                selection_yearpos = i;
            }
        }
        ...
        Systems.out.println("______**" + yeartouched + "__" + curyear + "__" + expyear + "__" + expmonth);
    ...
    private void callSkipApi() {
        ...
        try {
            j.put("id", SessionSave.getSession(PASS_ID, CardRegisterAct.this));
    ...
    private void setUIDisplay() {
        ...
        try {
            JSONArray jsonArray = new JSONArray(SessionSave.getSession(PASS_PAYMENT_OPTION, CardRegisterAct.this));
                ...
                if (validations(ValidateAction.isNullYear, CardRegisterAct.this, expyear))
                    if (validations(ValidateAction.isValidCvv, CardRegisterAct.this, cvvNumber))
                        if (!termsTxt.isChecked())
                            dialog = Utility.alert_view_dialog(CardRegisterAct.this, "Message", "" + NC.getResources().getString(R.string.agree_the_terms_and_condition), "" + NC.getResources().getString(R.string.ok), "", true, new DialogInterface.OnClickListener() {
                                @Override
                            ...
                            try {
                                ...
                                j.put("passenger_id", SessionSave.getSession(PASS_ID, CardRegisterAct.this));
                                final String url = "type=add_card_details";

                                CreditCardData creditCardData = new CreditCardData(cardName, "", "P", expmonth, expyear, CardNumber, "", "1", CardNumber, "");
                                new CardRegister(url, j, creditCardData);
                            } catch (Exception e) {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.back_text:
                showLoading(CardRegisterAct.this);
                final Intent i = new Intent(CardRegisterAct.this, LoginSelectionAcivity.class);
                startActivity(i);
                finish();
                break;
    ...
    private void cashApi() {
        ...
        if (!termscheck.isChecked())
            dialog = Utility.alert_view_dialog(CardRegisterAct.this, "Message", "" + NC.getResources().getString(R.string.agree_the_terms_and_condition),
    ...
    private void ShowPromoDilaog() {
        ...
        try {
            ...
            Colorchange.ChangeColor(mDialog.findViewById(R.id.alert_id), CardRegisterAct.this);
                ...
                public void onClick(final View v) {
                    ...
                    try {
                        ...
                        if (promocode.length() != 0) {
                            ...
                            dialog = Utility.alert_view_dialog(CardRegisterAct.this, "Message", "" + NC.getResources().getString(R.string.reg_enter_referral),
                                    "" + NC.getResources().getString(R.string.ok), "", true, new DialogInterface.OnClickListener() {
                                        @Override
                                        public void onClick(DialogInterface dialog, int which) {
                                            dialog.dismiss();
    ...
    private class SkipCard implements APIResult {
        private SkipCard(final String url, JSONObject data) {
            ...
            new APIService_Retrofit_JSON_NoProgress(CardRegisterAct.this, this, data, false).execute(url);
        }
            ...
            if (isSuccess) {
                try {
                    ...
                    if (json.getInt("status") == 1) {
                        mDialog.dismiss();
                        dialog = Utility.alert_view_dialog(CardRegisterAct.this, "Message", "" + json.getString("message"),
                                "" + NC.getResources().getString(R.string.ok), "", true, new DialogInterface.OnClickListener() {
                                    @Override
                                    public void onClick(DialogInterface dialog, int which) {
                                        dialog.dismiss();
                ...
                runOnUiThread(new Runnable() {
                    public void run() {
                        ShowToast(CardRegisterAct.this, NC.getString(R.string.server_con_error));
                    }
                });
            }
        }
    ...
    private class CardRegister implements APIResult {
        ...
        private CardRegister(final String url, JSONObject data, CreditCardData creditCardData) {
                ...
                try {
                    ...
                    if (json.getInt("status") == 1) {
                        if (creditCardData != null) {
                            creditCardData.setId(json.getString("passenger_cardid"));
                            creditCardData.setCard(json.getString("masked_creditcard_no"));
                            creditCardRepository.insertCreditCard(creditCardData);
                        }
                        SessionSave.saveSession(TaxiUtil.NEED_TO_COMPLETE_CARD_REG, false, CardRegisterAct.this);
                        try {
