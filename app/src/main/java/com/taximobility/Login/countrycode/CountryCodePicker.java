...
public class CountryCodePicker extends RelativeLayout {
    ...
    int LIB_DEFAULT_COUNTRY_CODE = 91;
...
//        Log.d(TAG, "Applying custom property");
        ...
        try {
            //hide nameCode. If someone wants only phone code to avoid_morebooking name collision for same country phone code.
...
//            defaultCountryNameCode = a.getString(R.styleable.CountryCodePicker_defaultNameCode);
            defaultCountryNameCode = SessionSave.getSession("country_iso_code", context);
            ...
            defaultCountryNameCode = SessionSave.getSession("country_iso_code", context);//IN
            defaultCountryCode = Integer.parseInt(SessionSave.getSession("country_code", context).replace("+", ""));//+91
            selectedCountry = Country.getCountryForNameCodeFromCustomMasterList(preferredCountries, customLanguage, defaultCountryNameCode);
            if (selectedCountry != null) {
                textView_selectedCountry.setText("+" + defaultCountryCode);
...
//                int defaultCountryCode = a.getInteger(R.styleable.CountryCodePicker_defaultCode, -1);
                int defaultCountryCode = Integer.parseInt(SessionSave.getSession("country_code", context));
            ...
            textView_selectedCountry.setText(SplashActivity.CURRENT_COUNTRY_CODE);
            defaultCountryNameCode = SplashActivity.CURRENT_COUNTRY_ISO_CODE;//IN
            defaultCountryCode = Integer.parseInt(SplashActivity.CURRENT_COUNTRY_CODE.replace("+", ""));//+91
        } finally {
            a.recycle();
        }
...
//        Log.d(TAG, "Setting selected country:" + selectedCountry.logString());
     ...
     *                                false: to avoid_morebooking auto pop of keyboard
     */
    ...
    void refreshPreferredCountries() {
        if (countryPreference == null || countryPreference.length() == 0) {
            ...
            for (String nameCode : countryPreference.split(",")) {
                ...
                if (country != null) {
                    if (!isAlreadyInList(country, localCountryList)) { //to avoid_morebooking duplicate entry of country
...
//            Log.d("custom master list", " has no country");
     ...
     * Custom master list will only limit the visibility of irrelevant country from selection dialog. But all other functions like
     * setCountryForCodeName() or setFullNumber() will consider all the countries.
     *
