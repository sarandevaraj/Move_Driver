...
public class APIService_Retrofit_JSON_NoProgress {
    ...
    public APIService_Retrofit_JSON_NoProgress(Context ctx, APIResult res, JSONObject j, boolean getmethod, int time) {
        mContext = ctx;
        response = res;
        this.data = j;
                ...
                if (!wholeURL) {
                    if (url_type.equalsIgnoreCase("getmodel_fare_details"))
                        coreResponse = client.coreDetailsg("no-cache", TaxiUtil.COMPANY_KEY,
                                url_type,SessionSave.getSession(PASS_ID,mContext));
                    else {
                        coreResponse = client.coreDetailsg("no-cache", TaxiUtil.COMPANY_KEY,
...
//                    client = new ServiceGenerator(mContext, dont_encode, timeOut).createService(CoreClient.class);
                ...
                Call<ResponseBody> coreResponse = client.updateUser(
                        TaxiUtil.COMPANY_KEY, body, url_type, SessionSave.getSession(LANG, mContext),SessionSave.getSession("PASS_ID", mContext));
                coreResponse.enqueue(new RetrofitCallbackClass<ResponseBody>(mContext, new Callback<ResponseBody>() {
                    @Override
                    public void onResponse(Call<ResponseBody> call, retrofit2.Response<ResponseBody> response) {
                        String data = null;
