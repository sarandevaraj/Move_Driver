...
import com.mayan.sospluginmodlue.countrycode.CountryCodePicker
import com.mayan.sospluginmodlue.countrycode.CountryCodePicker.CURRENT_COUNTRY_ISO_CODE
...
class SOSActivity : AppCompatActivity(), ItemClicked {
    ...
    private lateinit var submit_button: AppCompatButton
    lateinit var rv_contact_list: RecyclerView
    private lateinit var no_data_lay: LinearLayout
    lateinit var emergency: AppCompatButton
    lateinit var back: AppCompatImageView
    lateinit var more_info: TextView
    private lateinit var adding_progress: ContentLoadingProgressBar
    private lateinit var long_description: TextView
    private lateinit var short_description: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        print(
            "langggg" + SessionSave.getSession(
                "Lang", this@SOSActivity
            ) + ":" + SessionSave.getSession("Lang_Country", this@SOSActivity)
        )
        ...
        if (SessionSave.getSession("Lang", this@SOSActivity).equals("pr")) {
            langcountry = "pt_PT"
            val arry =
                langcountry.split("_".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            config.locale = Locale(arry[0], arry[1])
            Locale.setDefault(Locale(arry[0], arry[1]))
            this@SOSActivity.getBaseContext().getResources()
        ...
        submit_button = findViewById<AppCompatButton>(R.id.submit_button)
        rv_contact_list = findViewById<RecyclerView>(R.id.rv_contact_list)
        no_data_lay = findViewById<LinearLayout>(R.id.no_data_lay)
        emergency = findViewById<AppCompatButton>(R.id.emergency)
        back = findViewById<AppCompatImageView>(R.id.back)
        more_info = findViewById(R.id.more_info)
        adding_progress = findViewById<ContentLoadingProgressBar>(R.id.adding_progress)
        long_description = findViewById(R.id.long_description)
        ...
        rv_contact_list.layoutManager =
            LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false)

        rv_contact_list.adapter = ContactAdapter(contactsDataList, this, this)
        no_data_lay.visibility = View.GONE
        skeletonScreen =
            Skeleton.bind(rv_contact_list).adapter(rv_contact_list.adapter).shimmer(true).angle(20)
                .frozen(false).duration(1200).count(3).load(R.layout.sos__empty_list_item).show()
...
//        dialog.getWindow()?.setAttributes(lp)
        ...
        val forgetView: View =
            this.layoutInflater.inflate(R.layout.sos_contact_dialog_view, null)
        ...
        val addcontact = forgetView.findViewById(R.id.add_contact) as RelativeLayout
        ccp = forgetView.findViewById(R.id.ccp)
        println("country_iso_code in sos activity :${SessionSave.getSession("country_iso_code",this)}")
        if (!SessionSave.getSession("country_iso_code", this).equals("")) {
            ccp.setCountryForNameCode(SessionSave.getSession("country_iso_code", this));
        } else ccp.setCountryForNameCode(CURRENT_COUNTRY_ISO_CODE);
        addcontact.visibility = View.VISIBLE
        ...
        yesBtn.setOnClickListener {
            ...
            try {
                callAddApi(
                    mbl_number.text.toString(),
                    name.text.toString(),
                    ccp.textView_selectedCountry.text.toString().trim()
                  //  SessionSave.getSession("country_code", this@SOSActivity)
                )
                //                    no_data_lay?.visibility = View.GONE
        ...
        Handler().postDelayed({
            try {
                if (ActivityCompat.checkSelfPermission(
                        this@SOSActivity, Manifest.permission.ACCESS_FINE_LOCATION
                    ) != PackageManager.PERMISSION_GRANTED || ActivityCompat.checkSelfPermission(
                        this@SOSActivity, Manifest.permission.ACCESS_COARSE_LOCATION
                    ...
                    alert?.setOnShowListener {
                        alert.getButton(AlertDialog.BUTTON_NEGATIVE).setTextColor(
                            ContextCompat.getColor(
                                this@SOSActivity, R.color.button_accept
                            )
                        )
                        alert.getButton(AlertDialog.BUTTON_POSITIVE)
                            .setTextColor(ContextCompat.getColor(this@SOSActivity, R.color.black))
                    }
                    alert.show()
                } else {
    ...
    private fun callApi() {
        ...
        val request = ApiRequestData.getEmergencyRequestData(
            SessionSave.getSession(
                "user_type", this@SOSActivity
            ),
            SessionSave.getSession("sos_id", this@SOSActivity).toInt(),
            SessionSave.getSession("company_id", this@SOSActivity)
        )

        val loginResponse =
            client.emergencyList(request, SessionSave.getSession("Lang", this@SOSActivity))
        ...
        loginResponse.enqueue(object : Callback<EmergencyListData> {
            ...
            override fun onResponse(
                call: Call<EmergencyListData>,
                response: Response<EmergencyListData>
            ) {
                ...
                if (emergencyListData?.status == 1) {
                    ...
                    if (emergencyListData.detail.size > 0) {
                        ...
                        rv_contact_list.adapter =
                            ContactAdapter(contactsDataList, this@SOSActivity, this@SOSActivity)
    ...
    private fun callAddApi(phno: String, name: String, c_code: String) {
        ...
        val client = ServiceGenerator(this@SOSActivity, "").createService(CoreClient::class.java)
        val request = ApiRequestData.AddContactRequestData(
            SessionSave.getSession("sos_id", this@SOSActivity).toInt(),
            SessionSave.getSession("user_type", this@SOSActivity),
            phno,
            name,
            c_code,
            SessionSave.getSession("company_id", this@SOSActivity)
        ...
        val loginResponse =
            client.addContact(request, SessionSave.getSession("Lang", this@SOSActivity))
        showAddLoading()
            ...
            override fun onResponse(
                ...
                if (emergencyListData?.status == 1) {
                    ...
                    if (emergencyListData.detail.size > 0) {
                        ...
                        val s = Gson().toJson(contactsDataList, listOfTestObject)
                        SessionSave.saveSession("contact_sos_list", s, this@SOSActivity)
                        Toast.makeText(this@SOSActivity, "Contact added successfully", Toast.LENGTH_LONG)
                                .show()
                    } else {
                        errorMessage(getString(R.string.please_try_again))
                    }
                    ...
                    Toast.makeText(this@SOSActivity, emergencyListData?.message, Toast.LENGTH_LONG)
                        .show()
                    hideAddLoading()
                }
            }
    ...
    private fun callDeleteApi(id: Int) {
        ...
        val client = ServiceGenerator(this@SOSActivity, "").createService(CoreClient::class.java)
        val request = ApiRequestData.DeleteContactRequestData(
            SessionSave.getSession(
                "sos_id", this@SOSActivity
            ).toInt(),
            SessionSave.getSession("user_type", this@SOSActivity),
            id,
            SessionSave.getSession("company_id", this@SOSActivity)
        ...
        val loginResponse =
            client.deleteContact(request, SessionSave.getSession("Lang", this@SOSActivity))
...
//            makeRequest()
    ...
    private fun makeRequest() {
        ...
        alertBuilder.setPositiveButton(android.R.string.yes) { dialog, which ->
            ActivityCompat.requestPermissions(
                this, arrayOf(Manifest.permission.READ_CONTACTS), RECORD_REQUEST_CODE
            )
            dialog.dismiss()
        }
        ...
        alert!!.setOnShowListener {
            if (alert != null) {
                alert.getButton(AlertDialog.BUTTON_NEGATIVE)
                    .setTextColor(resources.getColor(R.color.button_accept))
                alert.getButton(AlertDialog.BUTTON_POSITIVE)
    ...
    public override fun onActivityResult(requestCode: Int, resultCode: Int, datas: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK && datas != null) {
            ...
            if (c!!.moveToFirst()) {
                ...
                if (hasPhone.equals("1", ignoreCase = true)) hasPhone = "true"
                else hasPhone = "false"
                ...
                phoneNumber = phoneNumber.replace("-", "").replace("\\s".toRegex(), "")
                    .replace(SessionSave.getSession("default", this@SOSActivity), "")
                ...
                if (isAlreadyPresent) {
                    ...
                    Toast.makeText(
                        this@SOSActivity, getString(R.string.already_added), Toast.LENGTH_LONG
                    ).show()
                } else {
                    println("numbereee nn  $phoneNumber")
                    if (phoneNumber.length < 7) Toast.makeText(
                        this@SOSActivity, getString(R.string.contact_min_alert), Toast.LENGTH_LONG
                    ).show()
                    else if (phoneNumber.length > 15) Toast.makeText(
                        this@SOSActivity, getString(R.string.contact_min_alert), Toast.LENGTH_LONG
                    ).show()
                    else callAddApi(
                        phoeNumberWithOutCountryCode(phoneNumber),
                        name,
    ...
    private fun countryCodefromPhoneNumber(phoneNumber: String): String {
        if (phoneNumber.contains("+") && phoneNumber.length > 10) {
            val phoneNumbers = phoneNumber.substring(0, phoneNumber.length - 10)
            println("numbereee countrycode  $phoneNumbers")
            return phoneNumbers
        } else return ""
    }

    private fun phoeNumberWithOutCountryCode(phoneNumber: String): String {
        if (phoneNumber.contains("+") && phoneNumber.length > 10) {
            val phoneNumbers = phoneNumber.substring(phoneNumber.length - 10, phoneNumber.length)
